...
Three axes. All three sound like "compiles to machine code", and they answer
independent questions, so a build can sit anywhere on each of them.
...
Native means the Cranelift path, with no C compiler in the middle for the
...
raw syscall. macOS uses `_start` too and routes through libSystem.
...
The generational pool reaches this axis because it is Frost. A fixed-capacity
slab lives inside a struct, with `malloc` out of the picture, so generational
storage works with no libc at all. See
...
and it is the oracle the differential tests compare against, so a
miscompilation in the Frost compiler has something to be caught by. Both roles
are scaffolding, and the bootstrap being ahead on a feature is a stage of the
work.

The Frost compiler holds to a three-stage fixpoint. It compiles its own source,
a compiler built from that output compiles the source again, and the two
