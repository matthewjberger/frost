...
type system that deliberately makes the dangerous shapes *unrepresentable*
rather than merely *checked*.

This document explains each guarantee, why it holds, and where it is enforced.
The short version: Frost removes the need for a borrow checker's hardest
machinery (lifetime inference, region variables) by making references
**second-class**. Everything else — moves, exclusivity, resource cleanup,
dangling-pointer freedom — follows from that one decision plus a small number of
local rules.

## The five guarantees

1. **No dangling references.** A reference can never outlive the value it points
   at, because references cannot be stored or returned — they exist only for the
...
static rules keep handles honest — a handle is plain copyable data, not a
reference, so the compiler never has to track its lifetime.

---

## 1. Second-class references → no dangling pointers
...
This is what lets the borrow analysis stay entirely scope-local.

The same rule is what makes `pool[handle]` sound (see §5): dereferencing a handle
yields a `&mut T` / `&T` borrow, and that borrow is second-class *for free* —
you cannot stash it in a struct or return it, so it cannot dangle past the
pool operation.
...
## 2. Move checking → no use-after-move
...
## 3. Borrow exclusivity → no mutable aliasing
...
This per-call check is *sufficient* — not merely necessary — because references
are second-class. A borrow cannot be squirreled away to be aliased later, so
there is no cross-call aliasing to reason about. The whole "who else holds a
reference to this?" question collapses to "what does this one call borrow?".

Enforced by `check_borrow_exclusivity` per call-site argument list.

## 4. Linear resources → no leaks, no double-free, non-ignorable errors
...
- **At most once** comes from the move checker (§2): consuming a linear value
  moves it, so a second use is a use-after-move error → no double-free.
- **At least once** is the new rule: a linear value that is still live at the
  end of the function that owns it is a "never consumed" error → no leak.
...
consumes it). This is how Frost **replaces `Drop`**: cleanup is an *obligation
the type system tracks*, not an implicit call inserted behind your back.

A powerful consequence: a `linear enum` returned from a fallible function is a
**non-ignorable error**. You cannot drop it on the floor, so a failure must be
matched (or otherwise consumed) — the type system makes silently swallowing an
error a compile error.

Enforced by the same `MoveChecker`, tracking which linear bindings remain live at
scope exit.

## 5. Generational handles → no use-after-free through the heap
...
generation)` pair — plain copyable data you *can* freely store and return
(unlike a reference).
...
used to read or corrupt whatever now occupies the slot. It is safe **use-after-
free detection** without a GC and without reference counting.
...
Crucially, the borrow you get is a **second-class reference** (§1). Storing it in
a struct or returning it is already rejected, so a handle-deref borrow cannot
...
and threading them through generics. Frost pays a different price up front —
references cannot escape — and in exchange deletes that entire machinery:

| Hazard                       | How Frost removes it                                   |
| ---------------------------- | ------------------------------------------------------ |
| Dangling reference           | References are second-class → cannot outlive the call  |
...
The analysis is a single AST pass; the only runtime cost is one integer compare
per handle access, and only when you choose to use pools.

## What is *not* yet guarded

Honesty about the current implementation:

- **Raw pointers** (`^T`) are an explicit escape hatch (used for FFI and the
  pool runtime's internals). They are `Copy` and unchecked, exactly like C
...
misuse, and that is what the five guarantees above deliver.
